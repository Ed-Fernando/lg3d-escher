...
public class Input {
  ...
  public void keyboard_mapping () {
	...
	for (int k = 0; k < keysyms_per_keycode; k++) {
	    keysyms[n++] = reply.read4 (replyOffset);
	    replyOffset += 4;
	}
    }
    ...
    /* 
    System.err.println("---------------------------------------------------");
    System.err.println("Escher Keysyms Mapping");
    System.err.println("keysyms_per_keycode = " + keysyms_per_keycode);
    n = 0;
    for (int i = 0; i < keysym_count; i++) {
	System.err.print("keysyms for modifier " + i + ":\t");
        for (int k = 0; k < keysyms_per_keycode; k++) {
	    System.err.print(keysyms[n++] + "\t");
	}
	System.err.println();
    }
    System.err.println("---------------------------------------------------");
  ...
  public Enum modifier_mapping () {
    ...
    Data reply = display.read_reply (request);
    return new Enum (reply, 32, reply.read1 (1)*8);
  }
